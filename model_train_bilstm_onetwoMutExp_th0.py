...
class CustomData(Dataset):
    def __init__(self, 
             ...
             exp_path_1mut = './train_data/exp_train_fullseq_th0.csv',
             exp_path_2mut = './test_data/twoPoint_mut_th0_1800.csv',
             flag='train',
             fold_idx=0,
             exp_weight=1,):
        ...
        expdata_1mut = pd.read_csv(exp_path_1mut) 
        expdata_2mut = pd.read_csv(exp_path_2mut)
        ...
        negative_samples = negative_samples.sample(n=round(1*len(positive_samples))) 

        selected_data_noexp = pd.concat([positive_samples, negative_samples])
        selected_data = pd.concat([selected_data_noexp, expdata_1mut, expdata_2mut])

        print(selected_data[selected_data['label']==1].shape)
        print(selected_data[selected_data['label']==0].shape)
        ...
        weights = np.ones(len(selected_data),dtype=np.float32)
        if flag == 'train':
            ...
            val_peptides = self.peptides[val_indices]
            val_labels = self.labels[val_indices]

            val_df = pd.DataFrame({'peptide': val_peptides, 'label': val_labels})
            val_df = val_df[~val_df['peptide'].isin(selected_data_noexp['peptide'])]

            self.peptides = val_df['peptide'].values
            self.labels = val_df['label'].values

            self.weights = weights[val_indices]
...
if __name__ == '__main__':
    ...
    for fold in range(5):
        ...
        model_dir = './output/models/BiLSTM_atten_noexp/'
        model_name = f'BiLSTM_atten_{fold}.model'
        ...
        # for epoch in (range(NUM_EPOCHS)):
            ...
            if val_auc > best_AUC:
                ...
                save_path = './output/models/BiLSTM_atten_onetwomut_th0/'
